...
if (isset($_REQUEST['submit-story'])) {
    // Check session
    if (!isset($_SESSION['userid'])) {
     ...
     $statusImage = $_FILES['status-image'];
     $id = $_SESSION['userid'];
    ...
    if ($statusImage['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExt, $allowed)) {
            if ($imageSize <= 10000000) { // 10MB limit
                ...
                if (!is_dir('status-img')) {
                    mkdir('status-img', 0777, true);
                }

                // Generate a unique file name
                $uniqueImageName = 'image_'.$id.'_'.uniqid() . '.' . $fileExt;
                $uploadPath = 'status-img/' . $uniqueImageName;

                // Move file to upload directory
                if (move_uploaded_file($imageTmpName, $uploadPath)) {
                    // Insert data into the database
                    $stmt = $conn->prepare("INSERT INTO stories (user_id, image_path, caption) VALUES (?, ?, ?)");
