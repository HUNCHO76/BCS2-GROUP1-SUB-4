...
if (isset($_POST['submit-update'])) {
    ...
    $id = $_GET['id']; // Ensure $id is passed via GET.
    ...
    if (empty($username) || empty($email)) {
        header("location: setting.php?error=emptyfield&id=" . $id);
        exit();
    }

    // Validate email and username
    if (!filter_var($email, FILTER_VALIDATE_EMAIL) || !preg_match("/^[a-zA-Z0-9]*$/", $username)) {
        header("location: setting.php?error=invalidinput&id=" . $id);
        exit();
    }

    // Check current profile picture in the database
    ...
    $imagePath = $currentImagePath; // Default to the current image
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($fileType, $allowedFileTypes)) {
            header("location: setting.php?error=invalidfiletype&id=" . $id);
            exit();
        }

        if ($fileSize > $maxFileSize) {
            header("location: setting.php?error=filetoolarge&id=" . $id);
            exit();
        }
        ...
        $imagePath = $uploadsDir . 'image_' . $id . '.' . $fileType;
        if (!move_uploaded_file($fileTmpPath, $imagePath)) {
            header("location: setting.php?error=uploadfailed&id=" . $id);
            exit();
        }
    }

    // Update the user record
    $sql = "UPDATE users SET username=?, email=?, bio=?, gender=?, relationship=?, profile_picture=? WHERE user_id=?";
    $stmt = mysqli_stmt_init($conn);

    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: setting.php?error=sqlierror&id=" . $id);
        exit();
    }

    // Bind and execute the query
    mysqli_stmt_bind_param($stmt, "ssssssi", $username, $email, $bio, $gender, $relationship, $imagePath, $id);
    mysqli_stmt_execute($stmt);

    // Redirect on success
    header("location: setting.php?update=success&id=" . $id);
    exit();
        ...
        <header class="z-[100] h-[--m-top] fixed top-0 left-0 w-full flex items-center bg-white/80 sky-50 backdrop-blur-xl border-b border-slate-200 dark:bg-dark2 dark:border-slate-800">
            ...
            <div class="flex items-center w-full xl:px-6 px-2 max-lg:gap-10">
              ...
              while ($row = mysqli_fetch_assoc($select_user)) {
                  ...
                  $profile_picture = $row['profile_picture']; // Fetch profile picture path
                        
                           echo" <div  class='rounded-full relative bg-secondery cursor-pointer shrink-0'>
                                <img src='$profile_picture' alt='' class='sm:w-9 sm:h-9 w-7 h-7 rounded-full shadow shrink-0'> 
                            </div>
                            <div  class='hidden bg-white rounded-lg drop-shadow-xl dark:bg-slate-700 w-64 border2'
                                uk-drop='offset:6;pos: bottom-right;animate-out: true; animation: uk-animation-scale-up uk-transform-origin-top-right '>
                                <a href='#'>
                                    <div class='p-4 py-5 flex items-center gap-4'>
                                        <img src='$profile_picture' alt='' class='w-10 h-10 rounded-full shadow'>
                                        <div class='flex-1'>
                                            <h4 class='text-sm font-medium text-black'>$username</h4>
                                            <div class='text-sm mt-1 text-blue-600 font-light dark:text-white/70'>$email</div>
                                        </div>
                                    </div>
                                </a>
                                <hr class='dark:border-gray-600/60'>
                                <nav class='p-2 text-sm text-black font-normal dark:text-white'>
                                    <a href='setting.php?id=$usrid'>
        ...
        <?php
            // <!-- tab user basic info -->
             ...
             while ($row = mysqli_fetch_assoc($select_user)) {
                 ...
                 $relationship = $row['Relationship'];
                 $profile_picture = $row['profile_picture']; // Fetch profile picture path
                 echo "<main id='site__main' class='2xl:ml-[--w-side]  xl:ml-[--w-side-sm] p-2.5 h-[calc(100vh-var(--m-top))] mt-[--m-top]'>
                             ...
                             <div class='flex md:gap-8 gap-4 items-center md:p-8 p-6 md:pb-4'>
                                 ...
                                 <div class='relative md:w-20 md:h-20 w-12 h-12 shrink-0'> 
                                     <label for='file' class='cursor-pointer'>
                                         <img id='img' src=".$profile_picture."  class='object-cover w-full h-full rounded-full' alt=''/>
                                     </label>
                                     ...
                                     <label for='file' class='md:p-1 p-0.5 rounded-full bg-slate-600 md:border-4 border-white absolute -bottom-2 -right-2 cursor-pointer dark:border-slate-700'>
                                         ...
                                         <input id='file' type='file' name='image' class='hidden' /></input>
                                     </label>
                                 </div>
             
                                 <div class='flex-1'>
                                     <h3 class='md:text-xl text-base font-semibold text-black dark:text-white'> $username </h3>
                                     <p class='text-sm text-blue-600 mt-1 font-normal'>$email</p>
                                 </div>
                             </div>
                                 ...
                                 <div>
                                    ...
                                    <div class='md:flex items-center gap-10'>
                                        ...
                                        <div class=flex-1 max-md:mt-4>
                                            <input type='text' name='username' placeholder='Monroe' class='lg:w-1/2 w-full' value='$username'>
                                        </div>
                                    </div>
                                    ...
                                    <div class='md:flex items-center gap-10'>
                                        ...
                                        <div class='flex-1 max-md:mt-4'>
                                            <input type='text' name='email' placeholder='info@mydomain.com' class='w-full' value='$email'>
                                        </div>
                                    </div> 
                                    ...
                                    <div class='md:flex items-start gap-10'>
                                        ...
                                        <div class='flex-1 max-md:mt-4'>
                                            <textarea class='w-full' name='bio' rows='5' placeholder='Inter your Bio' value ='$bio'>$bio</textarea>
                                            </div>
                                            </div> 
                                            ...
                                            <option value='0' " . ($relationship == 0 ? "selected" : "") . ">None</option>
                                            <option value='1' " . ($relationship == 1 ? "selected" : "") . ">Single</option>
                                            <option value='2' " . ($relationship == 2 ? "selected" : "") . ">In a relationship</option>
                                            <option value='3' " . ($relationship == 3 ? "selected" : "") . ">Married</option>
                                            <option value='4' " . ($relationship == 4 ? "selected" : "") . ">Engaged</option>
                                           </select>
                                            </div>
                                            </div>
                                        '
                                    ...
                                    <button type='submit' name='submit-update' class='button lg:px-10 bg-primary text-white max-md:flex-1'> Save <span class='ripple-overlay'></span></button>
                                </div>
